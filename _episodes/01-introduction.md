...
~~~
In case you decide to follow the lesson locally, you won't need to use `ssh` command. The same situation will occur if you are working in a R Studio terminal through AWS. In the later, you'll access the server with an user and password but in both cases you might need to install some dependencies or set up a work environment.
~~~
{: .callout}
